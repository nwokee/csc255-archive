...
class AVL {
    ...
    int calcSubTreeSize(node *p); //calculates the stsize of p based on children
    int height(node *p) const; // returns the height of p if it exists
    int getSubTreeSize(node *p) const; // returns the subTreeSize of p
    ...
    int getKeyAtRank(node *ptr, int rank) const;
    ...
    int getKeyAtRank(int rank, int &key) const;
};
