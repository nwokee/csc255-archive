...
class graph {
    private:
    int *lambda; // array of size n with dijkstra distances
    int *set; // array of size n to track which set each node is in
    ...
    const int vidNIL = -1;
    const int X = 0; // Used with “set”, this will mark a node in set X
    const int Y = 1; // Used with “set”, this will mark a node in set Y
    queue *q;
    ...
    // will populate lambda with the distances from node s to all other nodes
    void dijkstra(int s); 

    // If at least one node is in Y, this will return true with minV set to 
    // the index of a node in Y that has minimum value of all nodes in Y
    ...
    void dumpAllPairsShortestPath();

    // this will return true if sLabel and dLabel are 
    // both valid labels of real nodes It will also return 
    // the distance from node sLabel to node dLabel.
