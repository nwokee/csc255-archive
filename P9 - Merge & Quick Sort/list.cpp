#include <cstdio>
#include <iostream>
#include <cstdlib>
#include "list.h"

using namespace std;

void list::buildHeap() {
    // heapify every key from the last parent upwards
    int lastParent = (listCount - 2) / 2;
    for (int i = lastParent; i >= 0; i--) {
        heapify(i);
    }
}



void list::heapify(int index) {
    // start if index is in bounds
	if (index < heapSize) {
		// find the greatest value between the given index and its children
		// only if they exist
		int maxIDX = index;
		if ((left(index) < heapSize) && (keys[left(index)] > keys[maxIDX])) {
			maxIDX = left(index);
		}
		if ((right(index) < heapSize) && (keys[right(index)] > keys[maxIDX])) {
			maxIDX = right(index);
		}

		// if maxIDX got changed
		// swap the index with the child and recurse down
		if (maxIDX != index) {
			swap(keys[maxIDX], keys[index]);
			heapify(maxIDX);
		}
	}
}



void list::swap(int &x, int &y) {
    int temp = x;
    x = y;
    y = temp;
}



int list::left(int index) const {
	// multiply by 2 add 1 to get left child
	return (index * 2) + 1;
}



int list::right(int index) const {
	// multiply by 2 add 2 to get right child
	return (index * 2) + 2;
}



void list::merge(int *array, int *l, int lN, int *r, int rN) {
        // initialize all variables needed
        int iL = 0;
        int iR = 0;
        int iM = 0;

        // comb through both left and right lists, adding the smallest
        // value each time until one of the lists is empty
        while (iL < lN && iR < rN) {
            if (l[iL] <= r[iR]) {
                array[iM++] = l[iL++];
            } else {
                array[iM++] = r[iR++];
            }
        }

        // add the remainder of the non empty list
        if (iL == lN) {
            while(iR < rN) {
                array[iM++] = r[iR++];
            }
        } else {
           while(iL < lN) {
                array[iM++] = l[iL++];
            } 
        }
}



int list::partition(int *array, int p, int r) {
    // initialize partition trackers
    int i = p - 1;
    int j = r + 1;

    // swap every key that isn't on the right side of the partition
    int save = array[p];
    while  (i < j) {
        do {
            j--;
        } while (array[j] > save);
        do {
            i++;
        } while (array[i] < save);
        if (i < j) {
            swap(array[i], array[j]);
        }
    }

    return j;

}



void list::mergeSort(int *array, int n) {
    // only split and merge if theres more than one node in the sublist
    if (n > 1) {
        // split list
        int leftN = n / 2;
        int *left = new int[leftN];
        for (int i = 0; i < leftN; i++) {
            left[i] = array[i];
        }
        
        int rightN = n - leftN;
        int *right = new int[rightN];
        for (int i = leftN; i < n; i++) {
            right[i - leftN] = array[i];
        }

        //sort the left and right sides of the sublist and then merge them
        mergeSort(left, leftN);
        mergeSort(right, rightN);
        merge(array, left, leftN, right, rightN);
    }
}



void list::quickSort(int *array, int p, int r) {
    // if partition is valid, partition again sort the two new partitions
    if (p < r) {
        int q = partition(array, p, r);
        quickSort(array, p, q);
        quickSort(array, q+1, r);
    }
}



list::list(int capacity) {
    // initialize variables
    listCapacity = capacity;
    listCount = 0;
    keys = new int[capacity];
}




list::~list() {
    // delete the keys object
    if (keys) {
        delete [] keys;
        keys = NULL;
    }
}



bool list::insert(int key) {
    // check if list is full
    bool returnCode = listCount < listCapacity;

    if (returnCode) {
        // shift all items in list down one spot
        for (int i = listCount; i > 0; i--) {
            keys[i] = keys[i-1];
        }

        // insert key at the start of list and increase list size
        keys[0] = key;
        listCount++;
    }

    return returnCode;
}



bool list::add(int key) {
    // check if list is full
    bool returnCode = listCount < listCapacity;

    if (returnCode) {
        // append key to the end of the list and increase list size
        keys[listCount] = key;
        listCount++;
    }

    return returnCode;
}



bool list::insertAt(int index, int key) {
    // check if list is full and if index is in bounds
    bool returnCode = (listCount < listCapacity);
    returnCode = returnCode && (index > -1) && (index <= listCount);

    if (returnCode) {
        // shift all items ahead of index down one spot
        for (int i = listCount; i > index; i--) {
            keys[i] = keys[i-1];
        }

        // insert key at the index and increase list size
        keys[index] = key;
        listCount++;
    }

    return returnCode;
}



bool list::deleteAt(int index, int &key) {
    // check if list is empty and if index is in bounds
    bool returnCode = (listCount > 0) && (index > -1) && (index < listCount);

    if (returnCode) {
        // store value to be removed
        key = keys[index];

        // shift every element before index up one and adjust list size
        for (int i = index + 1; i < listCount; i++) {
            keys[i-1] = keys[i];
        }

        listCount--;
    }

    return returnCode;
}



bool list::deleteFirst(int &key) {
    // check if list is empty
    bool returnCode = listCount > 0;

    if (returnCode) {
        // store value to be removed
        key = keys[0];

        // shift every element up one and adjust list size
        for (int i = 1; i < listCount; i++) {
            keys[i-1] = keys[i];
        }

        listCount--;
    }

    return returnCode;
}



bool list::deleteLast(int &key) {
    // check if list is empty
    bool returnCode = listCount > 0;

    if (returnCode) {
        // store value to be removed
        key = keys[listCount - 1];

        // adjust list size
        listCount--;
    }

    return returnCode;
}



void list::clear() {
    // set listCount to zero, emptying the list
    listCount = 0;
}



bool list::readAt(int index, int &key) const {
    // check if list is empty and index is valid
    bool returnCode = (listCount > 0) && (index > -1) && (index < listCount);

    if (returnCode) {
        // store the intended value
        key = keys[index];
    }

    return returnCode;
}



void list::printIt(int limit) const {
    // re adjust limit in case limit < 1 or limit > listCount
    if (limit > listCount || limit < 1) {
        limit = listCount;
    }

    // print blanket statement
    cout << "Printing " << limit << " of " << listCount << " entries" << endl;

    // loop through list until limit and print each key
    for (int i = 0; i < limit; i++) {
        cout << "At index " << i << " there is key " << keys[i] << endl;
    }
}



void list::printItBackwards(int limit) const {
    // re adjust limit in case limit < 1 or limit > listCount
    if (limit > listCount || limit < 1) {
        limit = listCount;
    }

    // print blanket statement
    cout << "Printing " << limit << " of ";
    cout << listCount << " entries backwards" << endl;

    // loop through list until limit and print each key
    for (int i = 1; i <= limit; i++) {
        cout << "At index " << listCount - i << " there is key ";
        cout << keys[listCount - i] << endl;
    }

}



int list::getIndex(int key) const {
    // initialize returnCode
    int returnCode = -1;

    // adjust the returnCode if value is found
    for (int i = 0; i < listCount; i++) {
        if (keys[i] == key) {
            returnCode = i;
            break;
        }
    }

    return returnCode;
}



int list::getCapacity() const {
    return listCapacity;
}



int list::getCount() const {
    return listCount;
}



void list::bubbleSort() {
    // go through list, swapping items one by one 
    // until the list is fully sorted
    for (int i = listCount - 1; i > 0; i --) {
        // insert swap checker for the sort
        bool swapped = false;
        for (int j = 0; j < i; j ++) {
            if (keys[j] > keys[j + 1]) {
                // keys are out of order, swap them and update swap checker
                int temp = keys[j];
                keys[j] = keys[j + 1];
                keys[j + 1] = temp;
                swapped = true;
            }
        }
        if (!swapped) {
            // if no swaps were made we can end loop early
            break;
        }
    }
}



void list::selectionSort() {
    for (int i = 0; i < listCount - 1; i++) {
        // look for the index of the smallest key in the list
        int minIndex = i;
        for (int j = i; j < listCount; j++) {
            if (keys[j] < keys[minIndex]) {
                minIndex = j;
            }
        }
        // swap the smallest key at the current date line
        int temp = keys[i];
        keys[i] = keys[minIndex];
        keys[minIndex] = temp;
    }
}



void list::insertionSort() {
    for (int i = 1; i < listCount; i++) {
        // store key to be inserted and 
        // find where exactly it should be placed
        int save = keys[i];
        int j = i - 1;
        // for loop kept destroying j so we couldn't access it
        // outside to insert the key, switched the operation to
        // use a while loop instead
        while (j >= 0) {
            if (keys[j] > save) {
                // key to be inserted is smaller, shift and keep moving
                keys[j + 1] = keys[j];
            } else {
                // key needs to be inserted here, exit the loop
                break;
            }
            j--;
        }
        //store the new value
        keys[j+1] = save;
    }
}



void list::heapSort() {
    // set heapSize and convert list into maxHeap 
    heapSize = listCount;
    buildHeap();

    // swap the first and last values of the heap and re heapify
    // until heapSize is 0
    while (heapSize > 0) {
        heapSize--;
        swap(keys[0],keys[heapSize]);
        heapify(0);
    }
}



void list::mergeSort() {
    // call mergeSort recursion
    mergeSort(keys, listCount);
}



void list::quickSort() {
    // call quickSort recursion
    quickSort(keys, 0, listCount - 1);
}



bool list::isSorted() {
    bool returnCode = true;
    // loop through list to see if it is sorted
    // print out where it fails (if it does)
    for (int i = 0; i < listCount - 1; i++) {
        if (keys[i] > keys[i + 1]) {
            cout << "Failed at index " << i << endl;
            returnCode = false;
            break;
        }
    }

    return returnCode;
}