...
class list  {
    ...
    // zips 2 sorted arrays together in ascending order
    void merge(int *array, int *l, int lN, int *r, int rN); 
    
    // provides the index at which the array has been partitioned
    int partition(int *array, int p, int r);

    // recursion helpers for MSQS
    ...
    // implements merge sort (time complexity of O(nlogn))
    void mergeSort();

    // implements quick sort (time complexity of O(nlogn))
    void quickSort();
